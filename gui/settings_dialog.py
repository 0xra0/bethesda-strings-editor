...
from PySide6.QtCore import Qt, Slot, QTimer, QThread, Signal, QT_TRANSLATE_NOOP
...
class SettingsDialog(QDialog):
    ...
    _GENERAL_TIPS = [
        QT_TRANSLATE_NOOP("SettingsDialog", "Press F7 to jump instantly to the next untranslated string."),
        QT_TRANSLATE_NOOP("SettingsDialog", "Ctrl+Enter approves the current translation and advances to the next string."),
        QT_TRANSLATE_NOOP("SettingsDialog", "Ctrl+R rejects the current translation and marks it for retranslation."),
        QT_TRANSLATE_NOOP("SettingsDialog", "Ctrl+K opens the Command Palette — fuzzy-search any action without touching the mouse."),
        QT_TRANSLATE_NOOP("SettingsDialog", "Translation Memory pre-loads known translations from a previous file — matched strings are never sent to the AI."),
        QT_TRANSLATE_NOOP("SettingsDialog", "Batch Translate Folder (File menu) retranslates an entire directory of string files in one run."),
        QT_TRANSLATE_NOOP("SettingsDialog", "The QC dialog's 'Auto-Retranslate Issues' button queues all flagged strings for a single batch fix."),
        QT_TRANSLATE_NOOP("SettingsDialog", "Ctrl+Alt+K opens the Consistency Checker — find the same source string with different translations."),
        QT_TRANSLATE_NOOP("SettingsDialog", "Ctrl+Alt+G runs the Ukrainian gender agreement checker for adjective–noun mismatches."),
        QT_TRANSLATE_NOOP("SettingsDialog", "ти/ви register consistency is handled in the translation prompt — the AI keeps informal ти or formal ви consistent per speaker automatically."),
        QT_TRANSLATE_NOOP("SettingsDialog", "Focus Mode shows one string at a time full-screen — great for distraction-free reviewing."),
        QT_TRANSLATE_NOOP("SettingsDialog", "The Difficulty Estimator (0–100 score) helps you prioritise which strings need manual review."),
        QT_TRANSLATE_NOOP("SettingsDialog", "Drag and drop a .strings, .dlstrings, .ilstrings, .esp, .esm, or .ba2 file onto the window to open it."),
        QT_TRANSLATE_NOOP("SettingsDialog", "The Diff Viewer (word-level) shows exactly what changed between two game versions of the same file."),
        QT_TRANSLATE_NOOP("SettingsDialog", "The Glossary Manager ensures consistent terminology — add key terms and the AI will respect them in every call."),
        QT_TRANSLATE_NOOP("SettingsDialog", "Protected terms are replaced with unique tokens before the AI sees the string and restored afterward."),
        QT_TRANSLATE_NOOP("SettingsDialog", "The Audit Log records every file operation and batch without ever storing actual string content."),
        QT_TRANSLATE_NOOP("SettingsDialog", "Crash Recovery auto-saves progress periodically — if the app crashes, your work is offered on next launch."),
        QT_TRANSLATE_NOOP("SettingsDialog", "The NexusMods Browser lets you search, preview, and download translation mods without leaving the app."),
        QT_TRANSLATE_NOOP("SettingsDialog", "Version Comparison migrates unchanged translations from an old file to a new game version automatically."),
        QT_TRANSLATE_NOOP("SettingsDialog", "Load lore snippets in the Lore RAG Manager to give the AI contextual accuracy for faction names and world events."),
        QT_TRANSLATE_NOOP("SettingsDialog", "The Font Checker identifies characters missing from Starfield's Scaleform SWF font atlases."),
        QT_TRANSLATE_NOOP("SettingsDialog", "Ctrl+M opens the Macro Editor — record repetitive edits as named macros and replay them with one click."),
        QT_TRANSLATE_NOOP("SettingsDialog", "Translator Profiles let you define per-locale style rules and author metadata for each language."),
        QT_TRANSLATE_NOOP("SettingsDialog", "Session Manager (Ctrl+Shift+N) saves your search and filter state so you can resume exactly where you left off."),
        QT_TRANSLATE_NOOP("SettingsDialog", "The Plugin Validator dialog scans ESP/ESM files for NPC dialogue camera bugs before packaging."),
        QT_TRANSLATE_NOOP("SettingsDialog", "Shift+C copies the source text of the selected row; Shift+V pastes it into the translation column."),
        QT_TRANSLATE_NOOP("SettingsDialog", "The status bar shows Total / Done / Left % and an ETA countdown during AI translation batches."),
        QT_TRANSLATE_NOOP("SettingsDialog", "Advanced Search supports full regex across source and translation columns simultaneously."),
        QT_TRANSLATE_NOOP("SettingsDialog", "BA2 archives with multiple .strings entries show a picker so you can choose which file to open."),
        QT_TRANSLATE_NOOP("SettingsDialog", "The Claude Chat Panel (dock) lets you ask Claude about the selected string and apply its suggestion directly."),
        QT_TRANSLATE_NOOP("SettingsDialog", "TTS Preview synthesizes a read-out of your translation for timing comparison with the original game audio."),
        QT_TRANSLATE_NOOP("SettingsDialog", "The Dialogue Tree Viewer shows the Quest → Topic → Response hierarchy from an ESP/ESM file as an interactive tree."),
        QT_TRANSLATE_NOOP("SettingsDialog", "Pop out the string table to a second monitor via Window → Pop-out Table for multi-monitor workflows."),
        QT_TRANSLATE_NOOP("SettingsDialog", "The Visual Context Preview renders the selected string inside a faithful in-game UI widget mockup."),
        QT_TRANSLATE_NOOP("SettingsDialog", "The fine-tuned qcgemma4-st model checks 16 issue codes including GLOSSARY_MISMATCH, UNTRANSLATED, and REPETITION_ARTIFACT."),
        QT_TRANSLATE_NOOP("SettingsDialog", "Enable 'Protect English text' when translating RU→UK to keep English terminology untouched."),
        QT_TRANSLATE_NOOP("SettingsDialog", "English anchors such as 'To Ukrainian:' and 'To English:' in the Modelfile structure the model's output reliably."),
        QT_TRANSLATE_NOOP("SettingsDialog", "Increasing num_ctx uses more VRAM but lets the model see longer strings and richer system prompts."),
        QT_TRANSLATE_NOOP("SettingsDialog", "The Translation Cache (SHA-256 keyed) avoids retranslating identical strings across different files or sessions."),
        QT_TRANSLATE_NOOP("SettingsDialog", "xTranslator SST XML files can be imported and exported — string IDs are matched first, then source text."),
        QT_TRANSLATE_NOOP("SettingsDialog", "Consistency Checker's auto-replace rewrites all variants to your chosen canonical form in one click."),
        QT_TRANSLATE_NOOP("SettingsDialog", "Pre-load Translation Memory before a Batch Translate run to skip strings that are already translated."),
        QT_TRANSLATE_NOOP("SettingsDialog", "The Spell Checker supports Hunspell, spylls (pure Python), or a CLI fallback depending on what is installed."),
        QT_TRANSLATE_NOOP("SettingsDialog", "The Gender Checker uses a Ukrainian noun gender dictionary — extend the dictionary to improve detection coverage."),
        QT_TRANSLATE_NOOP("SettingsDialog", "Pop out the Translation Editor pane as a floating dock for a larger, more comfortable editing area."),
        QT_TRANSLATE_NOOP("SettingsDialog", "The Claude API key is stored with AES-256-GCM encryption via the system keyring — never in plaintext on disk."),
        QT_TRANSLATE_NOOP("SettingsDialog", "To retranslate only failed strings, open the QC dialog and click 'Auto-Retranslate Issues'."),
        QT_TRANSLATE_NOOP("SettingsDialog", "The Pre-Translation Estimator learns from your manual corrections — it improves automatically as you work."),
        QT_TRANSLATE_NOOP("SettingsDialog", "All keyboard shortcuts can be reassigned in Settings → Keyboard Shortcuts to match your personal workflow."),
        QT_TRANSLATE_NOOP("SettingsDialog", "The Lore RAG search tab lets you preview exactly which context snippets will be injected for a given string."),
        QT_TRANSLATE_NOOP("SettingsDialog", "Use the Register Checker to ensure you address the player consistently with either ти or ви throughout the whole file."),
        QT_TRANSLATE_NOOP("SettingsDialog", "The 'Protect proper nouns' option keeps faction, company, ship, and character names from being translated by the AI."),
        QT_TRANSLATE_NOOP("SettingsDialog", "Rejected strings are highlighted in red in the table so you can find them quickly for manual correction."),
        QT_TRANSLATE_NOOP("SettingsDialog", "The AI repetition artifact checker catches copy-paste loops and model hallucinations before they reach the player."),
        QT_TRANSLATE_NOOP("SettingsDialog", "Newline count mismatch detection ensures your translation preserves the same line breaks as the source."),
        QT_TRANSLATE_NOOP("SettingsDialog", "Russian character leakage detection flags any Cyrillic characters from the wrong script in a Ukrainian output."),
        QT_TRANSLATE_NOOP("SettingsDialog", "Export the Version Comparison report as HTML or CSV for review by other team members."),
        QT_TRANSLATE_NOOP("SettingsDialog", "OLLAMA_NUM_PARALLEL (set where 'ollama serve' launches — runit /etc/sv/ollama/run, a systemd drop-in, or your shell, not in this app) sets concurrent GPU slots; match it to the app's parallel workers for full two-stream throughput. Each slot pre-allocates a whole context window, so lower it if the model keeps reloading from VRAM eviction — restart Ollama after changing it."),
        QT_TRANSLATE_NOOP("SettingsDialog", "On AMD ROCm cards (gfx10xx, RX 6800/6700), set HSA_ENABLE_SDMA=0 where 'ollama serve' launches to fix GPU ring hangs when long batches freeze."),
        QT_TRANSLATE_NOOP("SettingsDialog", "OLLAMA_KV_CACHE_TYPE=q8_0 together with OLLAMA_FLASH_ATTENTION=1 roughly halve the Ollama server's KV-cache VRAM."),
    ]
